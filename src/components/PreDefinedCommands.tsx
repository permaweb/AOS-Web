...
export default function PreDefinedCommands({
  isOpen,
  setIsOpen,
  setSendingCommand,
  setUserCommand,
      ...
      {blueprintsList.map((item: any, index: number) => {
        return (
          <li key={index}>
            <button
              type="button"
              className="w-full bg-gray-100 hover:bg-gray-200 text-left rounded-md pt-2 pr-2 pb-2"
              onClick={() => handleClick(item)}
            >
              <span className="bg-black text-white p-2 rounded-l-md">
                .{item.command}
              </span>
